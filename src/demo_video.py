...
# Lógica del semáforo (la misma de predict.py)
...
def main():
    ...
    # Agarrar TODAS las fotos de test
    fotos = glob.glob(os.path.join(TEST_DIR, '*.png'))
    if not fotos:
        print("No se encontraron fotos en la carpeta test.")
        return

    # Leer la primera foto para sacar el tamaño base del video
    # Forzamos un tamaño estándar (ej: 800x400) para que el video quede prolijo
    VIDEO_WIDTH = 800
    VIDEO_HEIGHT = 400
    FPS = 30
    
    # Crear el escritor de video (.mp4)
    ...
    for img_path in fotos:
        ...
        for r in results:
            ...
            # Redimensionar para el video
            im_resized = cv2.resize(im_bgr, (VIDEO_WIDTH, VIDEO_HEIGHT))
            
            # Dibujar el cartel del semáforo
            cv2.putText(im_resized, estado, (20, 40), cv2.FONT_HERSHEY_SIMPLEX, 0.8, color, 2)
            
            # Escribir el frame varias veces para simular que la pieza se queda 1.5 segundos en pantalla
            frames_to_write = int(FPS * 1.5) 
            for _ in range(frames_to_write):
                out.write(im_resized)
                
            # Opcional: Agregar una pantallita negra de 0.2 segundos para simular el cambio de pieza
